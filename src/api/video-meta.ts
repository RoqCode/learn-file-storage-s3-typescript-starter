...
export async function handlerVideoGet(cfg: ApiConfig, req: BunRequest) {
  ...
  const videoSigned = dbVideoToSignedVideo(cfg, video);

  return respondWithJSON(200, videoSigned);
}
...
export async function handlerVideosRetrieve(cfg: ApiConfig, req: Request) {
  ...
  const videosSigned = videos.map((video) => dbVideoToSignedVideo(cfg, video));

  return respondWithJSON(200, videosSigned);
}
